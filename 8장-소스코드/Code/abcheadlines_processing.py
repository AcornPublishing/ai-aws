...
def main():
    ...
    s3_ilc = os.path.join(args['S3_INPUT_BUCKET'],
                          args['S3_INPUT_KEY_PREFIX'], args['S3_INPUT_FILENAME'])
    abcnewsdf = spark.read.option("header","true").csv(('s3://' + s3_ilc))
    ...
    s3_olc = os.path.join(args['S3_OUTPUT_BUCKET'], args['S3_OUTPUT_KEY_PREFIX'])
    news_save.write.option("delimiter", "\t").mode("append").csv('s3://' + s3_olc)
    ...
    file_to_upload = "jar:file:/tmp/model.zip"
    SimpleSparkSerializer().serializeToBundle(news_pl_fit, file_to_upload, news_ftrs_df)
